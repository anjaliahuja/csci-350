...
int findLine(char type, bool isSenator, int customer) {
  ...
  */
  if(type == 'a') {
    ...
    /* Picking a line */
    /*Write("Before Acquire 1", sizeof("Before Acquire 1"), ConsoleOutput); */
...
int main(){
	Write("TEST_2: Managers only read one from one Clerk's total money received, at a time, Total sales never suffers from a race condition", sizeof("TEST_2: Managers only read one from one Clerk's total money received, at a time,
  Total sales never suffers from a race condition"), ConsoleOutput); 
	Printf("Number of Customers = %d\n", sizeof("Number of Customers = %d\n"), NUM_CUSTOMERS);
	 ...
	 Printf("Number of PassportClerks = %d\n", sizeof("Number of PassportClerks = %d\n"), NUM_PASSPORTCLERKS);
